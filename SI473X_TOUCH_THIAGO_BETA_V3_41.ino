...
// - AGC ON teclado não identifica este estado (tecla não selecionada)
//
//
//
//
...
//=======================================================================================
void ICACHE_RAM_ATTR RotaryEncFreq() { //ICACHE_RAM_ATTR ?????
  //=======================================================================================
    ...
    if (FirstLayer) { //==================================================
      ...
      for (int n = 0 ; n <= lastButton; n++) {
        if ((x > (Xbutst + (bt[bt[n].ButtonNum].XButos))) and (x < Xbutst + (bt[bt[n].ButtonNum].XButos) + Xbutsiz) and (y > Ybutst + (bt[bt[n].ButtonNum].YButos)) and (y < Ybutst + (bt[bt[n].ButtonNum].YButos) + Ybutsiz)) {
          ...
          if (n == HAM) {
            delay(DelayButton);  //HamBand button
            HamBand = true;
            HamBandlist();
            FirstLayer = false;
            SecondLayer = true;
      ...
      if (FREQbut == true) {
        ...
        for (int n = 0 ; n < 12; n++) { // which keys are pressed?
          if ((x > (Xbutst + (bt[bt[n].ButtonNum].XButos))) and (x < Xbutst + (bt[bt[n].ButtonNum].XButos) + Xbutsiz) and (y > Ybutst + (bt[bt[n].ButtonNum].YButos)) and (y < Ybutst + (bt[bt[n].ButtonNum].YButos) + Ybutsiz)) {
            ...
            if (n == 11) {// Send button
              delay (DelayButton); //Aguarda na tela a frequencia digitada
    ...
    if (ForthLayer) { //===============================================================
      if (BroadBand == true) {
        for (int n = 0 ; n <= lastBroad - 11; n++)
        {
          if ((x > (Xbutst + (bt[bt[n].ButtonNum].XButos))) and (x < Xbutst + (bt[bt[n].ButtonNum].XButos) + Xbutsiz) and (y > Ybutst + (bt[bt[n].ButtonNum].YButos)) and (y < Ybutst + (bt[bt[n].ButtonNum].YButos) + Ybutsiz))
          {
            delay(DelayButton); // XXXXXXXXXXXXXXXXXXXXXXX
            BroadBand = false;
            n = n + 11;
            bandIdx = bb[n].BbandNum;
            if ((bandIdx == 0) and (currentAGCgain >= 28)) currentAGCgain = previousAGCgain = 26; // currentAGCgain in FM max. 26
...
//=======================================================================================
{
  uint16_t Xbat = Xinfo + 433;
  uint16_t Ybat = Yinfo;

  tft.drawRoundRect(Xbat + 1, Ybat + 1, 22, 13, 1, TFT_BLACK); // Draw battery body
  tft.drawRoundRect(Xbat, Ybat , 24, 15, 2, TFT_BLACK);
  tft.fillRect(Xbat + 23, Ybat + 4, 3, 7, TFT_BLACK); // Draw positive

  tft.fillRect(Xbat + 4, Ybat + 4, 4, 7, TFT_BLACK); // Draw 30% battery
  tft.fillRect(Xbat + 10, Ybat + 4, 4, 7, TFT_BLACK); // Draw 60% battery
  tft.fillRect(Xbat + 16, Ybat + 4, 4, 7, TFT_BLACK); // Draw 90% battery
}
