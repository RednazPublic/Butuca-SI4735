...
// - What is the best delay after press a button (layer First, second, thirf, fourth): 400, 300, 200 ... None
...
Button bt[] = {       //
  ...
  { "FREQ"  ,  1 , "SEEK -"   , 6 , Xbut1 , Ybut1  },
  { "AGC"   ,  2 , "INFO"     , 2 , Xbut2 , Ybut2  },        //                             |----|
  { "STEP"  ,  3 , "RDS"      , 7 , Xbut3 , Ybut3  },        //                             | 12 |
  { "VOL"   ,  4 , "AGC +"    , 1 , Xbut4 , Ybut4  },        //                             |----|
  { "MEM"   ,  5 , "On Off"   , 10 , Xbut5 , Ybut5  },       //                             | 13 |
  { "BAND"  ,  6 , ""         , 8 , Xbut6 , Ybut6  },        //                             |----|
  { "MODE"  ,  7 , "STORE"   , 3 , Xbut7 , Ybut7  },         //                             | 14 |
  { "BFO"   ,  8 , "COLOR"    , 4 , Xbut8 , Ybut8  },        //                             |----|
  { "FILTER",  9 , "SPCTR"    , 5 , Xbut9 , Ybut9  },        //
  { "MUTE"  , 10 , ""         , 9 , Xbut10, Ybut10 },        //    |----|----|----|----|----|----|
  { "NEXT"  , 11 , "BACK"     , 11 , Xbut11, Ybut11 },       //    |  0 |  1 |  2 |  3 |  4 |  5 |
  { "SEEK +", 12 , "SEEK +"   , 12 , Xbut12, Ybut12 },       //    |----|----|----|----|----|----|
  { "SEEK -", 13 , "SEEK -"   , 13 , Xbut13, Ybut13 },       //    |  6 |  7 |  8 |  9 | 10 | 11 |
  { "SPCTR",  14 , "CLEAR"    , 14 , Xbut14, Ybut14 },       //    |----|----|----|----|----|----|
};
...
#define TFT_CHUMBO tft.color565(27, 27, 27)
#define TFT_DARKYELLOWBACK  tft.color565(127,99,0) //Converte RGB no padrão Color565 - RGB 7f6300
#define TFT_YELLOWUNSET  tft.color565(102,80,00) //Converte RGB no padrão - RGB CCA519
#define TFT_REDMETER  tft.color565(150, 21, 9) //Converte RGB no padrão Color565 - RGB 961509
...
void setup() {
  ...
  //  tft.drawBitmap (25, 140, Butuca, 100, 68, TFT_WHITE);
...
void BaselineDisplay ()  {
  ...
  for (int i = 1; i <= 7; i++) {
    ...
    if ((i == 2) || (i == 4) || (i == 6))
    {
      tft.setTextColor(TFT_REDMETER, BackColor);
      tft.print("+");
      tft.print(i * 10);
      tft.setTextColor(TextColor, BackColor);
    }
  ...
  tft.setTextColor(TFT_CHUMBO, BackColor);
  ...
  tft.setTextColor(TFT_YELLOWUNSET, BackColor);
...
void Smeter() {
  ...
  {
    tft.fillRoundRect(Xsmtr + 1 + (i * 12), Ysmtr + 12, 9, 11, 2, ((i > 9) ? TFT_REDMETER : TFT_YELLOWUNSET));
  }
...
void SNRmeter() {
  ...
  for (int i = 0; i <= spoint; i++)
  {
    tft.fillRoundRect(Xsmtr + 225 + (i * 12), Ysmtr + 12, 9, 11, 2, ((i > 9) ? TFT_BLACK : TFT_YELLOWUNSET));
  }
      ...
      if (FREQbut == true) {
        ...
        for (int n = 0 ; n < 12; n++) { // which keys are pressed?
          if ((x > (Xbutst + (bt[bt[n].ButtonNum].XButos))) and (x < Xbutst + (bt[bt[n].ButtonNum].XButos) + Xbutsiz) and (y > Ybutst + (bt[bt[n].ButtonNum].YButos)) and (y < Ybutst + (bt[bt[n].ButtonNum].YButos) + Ybutsiz)) {
            ...
            if (n == 11) {// Send button
              delay (300); //Aguarda na tela a frequencia digitada
              ...
              tft.setFreeFont(&ERRORTYPE);
              tft.setTextColor(BackColor, TFT_REDMETER);
    ...
    if (ForthLayer) { //===============================================================
      if (BroadBand == true) {
        for (int n = 0 ; n <= lastBroad - 11; n++)
        {
          if ((x > (Xbutst + (bt[bt[n].ButtonNum].XButos))) and (x < Xbutst + (bt[bt[n].ButtonNum].XButos) + Xbutsiz) and (y > Ybutst + (bt[bt[n].ButtonNum].YButos)) and (y < Ybutst + (bt[bt[n].ButtonNum].YButos) + Ybutsiz))
          {
            delay(400); // XXXXXXXXXXXXXXXXXXXXXXX
            BroadBand = false;
            n = n + 11;
            bandIdx = bb[n].BbandNum;
            if ((bandIdx == 0) and (currentAGCgain >= 28)) currentAGCgain = previousAGCgain = 26; // currentAGCgain in FM max. 26
...
void DrawButThla() { // Buttons Third layer
  ...
  for (int n = 0 ; n <= lastButton; n++)
  {
    if (n == 12 || n == 13 | n == 14)
    {
      tft.drawRoundRect(bt[bt[n].ButtonNum1].XButos + Xbutst, bt[bt[n].ButtonNum1].YButos + Ybutst - (n - 25), Xbutsiz - 2, YbutsizExtra - 2, RoundCorner, TFT_BLACK);
      tft.drawRoundRect(bt[bt[n].ButtonNum1].XButos + Xbutst, bt[bt[n].ButtonNum1].YButos + Ybutst - (n - 25), Xbutsiz - 3, YbutsizExtra - 3, RoundCorner - 1, TFT_BLACK);
...
void drawVOL()   {
  ...
  if (VOLbut) {
    ...
    tft.fillRoundRect((bt[bt[4].ButtonNum].XButos + Xbutst + 1) , (bt[bt[4].ButtonNum].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, TFT_DARKYELLOWBACK);
  } else {
    VOLbutcol = BackColor;
...
void DrawAGCgainbut()  {
  ...
  if (ThirdLayer)  {
    int AGCgainbutcol;
    if (AGCgainbut) {
      AGCgainbutcol = TFT_DARKYELLOWBACK;
      tft.drawRoundRect(bt[bt[4].ButtonNum1].XButos + Xbutst, bt[bt[4].ButtonNum1].YButos + Ybutst , Xbutsiz - 2, Ybutsiz - 2, RoundCorner, TFT_BLACK);
      tft.fillRoundRect((bt[bt[4].ButtonNum1].XButos + Xbutst + 1) , (bt[bt[4].ButtonNum1].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, TFT_DARKYELLOWBACK);
    } else {
      AGCgainbutcol = BackColor;
      tft.fillRoundRect((bt[bt[4].ButtonNum1].XButos + Xbutst + 1) , (bt[bt[4].ButtonNum1].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, BackColor);
      tft.drawRoundRect(bt[bt[4].ButtonNum1].XButos + Xbutst, bt[bt[4].ButtonNum1].YButos + Ybutst , Xbutsiz - 2, Ybutsiz - 2, RoundCorner, TFT_BLACK);
      tft.drawRoundRect(bt[bt[4].ButtonNum1].XButos + Xbutst, bt[bt[4].ButtonNum1].YButos + Ybutst , Xbutsiz - 3, Ybutsiz - 3, RoundCorner - 1, TFT_BLACK);
    }
    tft.setFreeFont(&ButtonText1);
    tft.setTextColor(TFT_BLACK, AGCgainbutcol);
    tft.setTextDatum(BC_DATUM);
    tft.drawString((bt[4].ButtonNam1), ( bt[bt[4].ButtonNum1].XButos + Xbutst + (Xbutsiz / 2 - 2)), (bt[bt[4].ButtonNum1].YButos + Ybutst  + (Ybutsiz / 2 + 12)));
    tft.setTextDatum(TL_DATUM);
    tft.setFreeFont(NULL);
  }
}
...
void DrawRDSbut()  {
  ...
  if (RDS) {
    RDSbutcol = TFT_DARKYELLOWBACK;
    tft.drawRoundRect(bt[bt[3].ButtonNum1].XButos + Xbutst, bt[bt[3].ButtonNum1].YButos + Ybutst , Xbutsiz - 2, Ybutsiz - 2, RoundCorner, TFT_BLACK);
    tft.fillRoundRect((bt[bt[3].ButtonNum1].XButos + Xbutst + 1) , (bt[bt[3].ButtonNum1].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, TFT_DARKYELLOWBACK);
    tft.drawString("ON", Xinfo + 358, Yinfo + 17);
  } else {
    RDSbutcol = BackColor;
    tft.fillRoundRect((bt[bt[3].ButtonNum1].XButos + Xbutst + 1) , (bt[bt[3].ButtonNum1].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, BackColor);
    tft.drawRoundRect(bt[bt[3].ButtonNum1].XButos + Xbutst, bt[bt[3].ButtonNum1].YButos + Ybutst , Xbutsiz - 2, Ybutsiz - 2, RoundCorner, TFT_BLACK);
    tft.drawRoundRect(bt[bt[3].ButtonNum1].XButos + Xbutst, bt[bt[3].ButtonNum1].YButos + Ybutst , Xbutsiz - 3, Ybutsiz - 3, RoundCorner - 1, TFT_BLACK);
  ...
  tft.setTextDatum(BC_DATUM);
  tft.drawString((bt[3].ButtonNam1), ( bt[bt[3].ButtonNum1].XButos + Xbutst + (Xbutsiz / 2 - 2)), (bt[bt[3].ButtonNum1].YButos + Ybutst  + (Ybutsiz / 2 + 12)));
  tft.setTextDatum(TL_DATUM);
  tft.setFreeFont(NULL);
}
...
void drawMUTE()  {
  ...
  if (Mutestat) {
    ...
    tft.fillRoundRect((bt[bt[10].ButtonNum].XButos + Xbutst + 1) , (bt[bt[10].ButtonNum].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, TFT_DARKYELLOWBACK);
  } else {
...
void drawAGC()  {
  ...
  if (si4735.isAgcEnabled()) {
    ...
    tft.fillRoundRect((bt[bt[2].ButtonNum].XButos + Xbutst + 1) , (bt[bt[2].ButtonNum].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, TFT_DARKYELLOWBACK);
  } else {
    AGCbutcol = BackColor;
...
void drawBFO ()  {
  ...
  if (bfoOn) {
    ...
    tft.fillRoundRect((bt[bt[8].ButtonNum].XButos + Xbutst + 1) , (bt[bt[8].ButtonNum].YButos + Ybutst + 1), (Xbutsiz - 4) , Ybutsiz - 4, RoundCorner, TFT_DARKYELLOWBACK);
  } else {
    BFObutcol = BackColor;
...
void subrstatus() {
  ...
  while (x == 0) {
    ...
    else tft.drawString("Freq. : " + String(currentFrequency / 100, 1) + " MHz", 150, 30);
    si4735.getCurrentReceivedSignalQuality();
    tft.drawString("RSSI : " + String(si4735.getCurrentRSSI()) + "dBuV", 150, 50); // si4735.getCurrentSNR()
    tft.drawString("SNR : " + String(si4735.getCurrentSNR()) + "uV", 150, 70);
    if (  currentMode == FM ) {
      snprintf(buffer, sizeof(buffer), "%s", (si4735.getCurrentPilot()) ? "STEREO" : "MONO");
      tft.drawString("         : " + String(buffer), 150, 90);
    }
    si4735.getAutomaticGainControl();
    si4735.getCurrentReceivedSignalQuality();
    tft.drawString("LNA GAIN : " + String(si4735.getAgcGainIndex()) + "/" + String(currentAGCAtt), 150, 110);
    tft.drawString("Volume : " + String(si4735.getVolume()), 150, 130);
    snprintf(buffer, sizeof(buffer), "%s", (si4735.isAgcEnabled()) ? "AGC ON " : "AGC OFF");
    tft.drawString(buffer, 150, 150);
    if (bfoOn) tft.drawString("BFO ON  ", 150, 170);
    else tft.drawString("BFO OFF ", 150, 190);
    tft.drawString("AVC max GAIN : " + String(si4735.getCurrentAvcAmMaxGain()), 150, 210);
    tft.drawString("Ant. Cap = " + String(si4735.getAntennaTuningCapacitor()) , 150, 230);
    tft.setTextColor(TextColor, BackColor);
    tft.drawString("BANDA : " + String(bandIdx) + "  " + String(band[bandIdx].bandName) , 150, 250);
    tft.drawString("FILTRO SSB : " + String(bandwidthSSB[bwIdxSSB]) + " kHz", 150, 270);
    tft.drawString("FILTRO AM : " + String(bandwidthAM[bwIdxAM]) + " kHz", 150, 290);
    tft.drawString("PASSO : " + String(currentStep), 150, 310);
    ...
    tft.drawString("BY RICARDO - GERT - THIAGO - ZANDER", 160, 225);
    //    tft.setFreeFont(NULL);
...
//=======================================================================================
{
  ...
  for (int i = 21; i <= 459; i++)
  {
    ...
    if (PointRSSI > 50)
    {
      tft.drawLine (i, 219, i, 219 - (PointRSSI + 100), TFT_REDMETER);
    }
    else
    {
      tft.drawLine (i, 219, i, 219 - 3 * PointRSSI, TFT_REDMETER);
    }
    si4735.frequencyUp();
...
//=======================================================================================
{
  ...
  {
    NightLight = false;
    BackColor  = tft.color565(227, 183, 23);
  }
  else
  {
    NightLight = true;
